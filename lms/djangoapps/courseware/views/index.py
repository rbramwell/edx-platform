...
from lms.djangoapps.gating.api import get_entrance_exam_percentage, get_entrance_exam_usage_key
...
class CoursewareIndex(View):
    ...
    def _add_entrance_exam_to_context(self, courseware_context):
        """
        Adds entrance exam related information to the given
        courseware context.
        """
        if course_has_entrance_exam(self.course):
            if getattr(self.chapter, 'is_entrance_exam', False):
                courseware_context['entrance_exam_passed'] = user_has_passed_entrance_exam(
                    self.effective_user,
                    self.course,
                )
                if not courseware_context['entrance_exam_passed']:
                    course_grade = CourseGradeFactory().create(self.effective_user, self.course)
                    courseware_context['entrance_exam_current_score'] = get_entrance_exam_percentage(
                        course_grade,
