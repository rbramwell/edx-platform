...
def _get_subsection_percentage(subsection_grade):
    ...
    """
    return _calculate_percentage(subsection_grade.graded_total.earned, subsection_grade.graded_total.possible) * 100.0


def _calculate_percentage(earned, possible):
    """
...
def evaluate_entrance_exam(course_grade, user):
    ...
    if milestones_helpers.is_entrance_exams_enabled() and getattr(course, 'entrance_exam_enabled', False):
        if get_entrance_exam_content(user, course):
            ...
            exam_percentage = get_entrance_exam_percentage(course_grade, exam_chapter_key)
            if exam_percentage >= course.entrance_exam_minimum_score_pct:
...
def get_entrance_exam_percentage(course_grade, exam_chapter_key):
    """
    Returns the percentage value for the given chapter.
    """
    score = 0.0
    chapter_grade = course_grade.chapter_grades.get(exam_chapter_key, None)
    if chapter_grade:
        earned = 0.0
        possible = 0.0
        ...
        score = _calculate_percentage(earned, possible)
    else:
